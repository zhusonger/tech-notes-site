...
- **内容只有一个来源**：后台数据库。首次启动只灌站点配置（品牌名、版块），不灌示例文章与项目 ——
  全新库前台是空的，内容靠后台录入；接口挂掉时前台回落到 `shared/content.mjs` 的兜底结构并明示降级，
  不会白屏也不会展示过期内容。
...
1. **`shared/content.mjs` 是唯一的结构源**：服务端首次启动只用它补站点配置与版块
   （`db.mjs` 的 `seedIfEmpty`），文章 / 项目 / 分类 / 标签不灌 —— 全新库前台为空，
   内容靠后台录入；前台也用它生成兜底内容。配置种子与兜底同源，结构上不可能分叉。
...
- 全新库启动后前台为空（文章 / 项目 / 分类 / 标签需后台录入）；仓库内的简历与配图为演示素材，需按自己的情况替换。
